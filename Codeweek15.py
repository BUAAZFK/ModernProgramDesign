...
def GetMusicInfo(id, cat):
	...
	songs = []
	# 获取歌单中可以看见的20首歌(只能获取到10首，因为lxml解析的问题)
	songs_info = soup.find('table', 'm-table', limit=20).find_all('tr')
	songs = {}
	for song in songs_info:
		temp = {}
		# <a href="/song?id=1998819725"><b title="My Only Wish (feat. Christopher)">My<div class="soil">gV</div> Only Wish (feat. Christopher)</b></a>
		songid_pattern = re.compile(r'.*id=(.*?)".*')
		song_id = songid_pattern.findall(str(song.select('.txt > a')[0]))
		temp['id'] = song_id
		author = song.select('.text > span')[0]['title']
		temp['author'] = author
		# 使用正则表达式提取
		# song_title = str(song.find('b'))
		# # <b title="My Only Wish (feat. Christopher)">My Onl<div class="soil">nm</div>y Wish (feat. Christopher)</b>
...
def WriteToDB(songInfos):
	with psycopg.connect("dbname=WangYiMusic user=postgres password=zfk660660") as conn:
		cur = conn.cursor()
		# 创建表
		try:
			cur.execute("create table voa (id varchar primary key, title varchar, author varchar, album varchar, mp3_path varchar)")
		except:
			print("表voa已存在！！")
			conn.rollback()
		finally:
			# 插入值
			for item in songInfos:
				cur.execute("insert into voa (id, title, author, album, mp3_path) values (%s, %s, %s, %s, %s)", (item['id'],item['title'],item['author'],item['album'],item['mp3_path']))
		cur.close()
